...
(() => {
    ...
    function updateHeader() {
        ...
        elements.subtitle.textContent = total
            ? `${total} review${total === 1 ? '' : 's'} saved. Select one to read the full review and view photos.`
    ...
    function renderColumnPicker() {
        ...
        elements.columnSummary.textContent = `${selectedKeys.length} columns`;
    }
    ...
    function createReviewRow(review) {
        ...
        number.textContent = `${review.rating || 0} / 5`;
        const stars = document.createElement('span');
        stars.className = 'score-stars';
        stars.setAttribute('aria-hidden', 'true');
        stars.textContent = `${'★'.repeat(review.rating)}${'☆'.repeat(Math.max(0, 5 - review.rating))}`;
        rating.append(number, stars);
    ...
    function openExportDrawer() {
        ...
        window.setTimeout(() => {
            elements.exportDrawer.querySelector('summary')?.focus({ preventScroll: true });
        }, 0);
    }
